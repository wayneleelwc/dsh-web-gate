...
[![Tests: passing](https://img.shields.io/badge/tests-64%20passing-brightgreen.svg)](#测试)
...
64 个测试覆盖哈希往返与篡改、token 签发/过期/pv 失效/吊销、限流锁定与清理、配置校验、CLI、登录/登出/改密、SSE 流式透传、WebSocket 鉴权与透传、头部处理（X-Forwarded-*）与安全响应头。
